...
def select_indices(
    ...
    *,
    ...
    if method == "stride":
        ...
        indices = np.arange(skip, total, stride, dtype=int)
    ...
    if method == "uniform":
        return np.linspace(skip, total - 1, samples, dtype=int)
    if method == "random":
        return np.sort(np.random.default_rng(seed).choice(np.arange(skip, total), samples, replace=False))
